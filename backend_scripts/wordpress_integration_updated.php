...
// IMPORTANTE: Cambiar este ID por el del producto creado
...
function mecha_procesar_cotizacion_3d()
{
    ...
    $thumbnailUrl = '';
    if (isset($cotizacion['thumbnailUrl']) && !empty($cotizacion['thumbnailUrl'])) {
        $thumbnailUrl = $cotizacion['thumbnailUrl'];
    } elseif (isset($cotizacion['thumbnail_url']) && !empty($cotizacion['thumbnail_url'])) {
        $thumbnailUrl = $cotizacion['thumbnail_url'];
    } elseif (isset($data['thumbnailUrl']) && !empty($data['thumbnailUrl'])) {
...
function mecha_guardar_datos_3d_orden($item, $cart_item_key, $values, $order)
{
    if (isset($values['impresion_3d']) && $values['impresion_3d'] === true) {
        ...
        if (isset($values['thumbnail_url']) && !empty($values['thumbnail_url'])) {
            $item->add_meta_data('_thumbnail_url', $values['thumbnail_url'], true);
        }
...
function mecha_mostrar_link_archivo_admin($item_id, $item, $product)
{
    ...
    $thumb_url = $item->get_meta('_thumbnail_url');
    if (!empty($thumb_url)) {
        echo '<p><img src="' . esc_url($thumb_url) . '" style="width: 50px; height: 50px; object-fit: contain; border: 1px solid #ccc; background:#fff;" /></p>';
    }
}
...
add_filter('woocommerce_cart_item_thumbnail', 'mecha_imagen_custom_carrito', 10, 3);
function mecha_imagen_custom_carrito($thumbnail, $cart_item, $cart_item_key)
{
    if (isset($cart_item['thumbnail_url']) && !empty($cart_item['thumbnail_url'])) {
        return '<img src="' . esc_url($cart_item['thumbnail_url']) . '" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail" alt="Vista Previa 3D" style="width: 100px; height: 100px; object-fit: contain; border-radius: 8px; background: #fff;" />';
    }
    return $thumbnail;
}
?>